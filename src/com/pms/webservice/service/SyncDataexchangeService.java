...
//		//1. check if sync request 
		...
		try{
			if( this.getEcs() == null ) {
				throw new Exception("数据交换请求参数不正确。");
			}
			...
			result = generateExchangeResponseOfExceptionResult(e.getMessage());
		}
		return result;
	}
	...
	private String generateExchangeResponseOfExceptionResult(String exceptionMessage) throws IOException {
		String result = null;
		Document doc = null;
		try{
			Element message = null;// dataset = null, data = null, condition = null, item = null;
			...
			data010004.addContent(item010004);
			itemSetAttribute(item010004, "key", "I010009");
			itemSetAttribute(item010004, "val", exceptionMessage);
			itemSetAttribute(item010004, "rmk", "备注");
